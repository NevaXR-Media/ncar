...
data class NVhalProperty<Raw, T>(val key: NVhalKey, val transform: (CarPropertyValue<Raw>) -> T) : NCarProperty<T> {
    ...
    fun optional() = NVhalProperty<Raw?, T?>(key) { propertyValue -> propertyValue.value?.let { _ -> transform(propertyValue as CarPropertyValue<Raw>)} }
    ...
    companion object {
        ...
        inline fun <reified T> array(key: NVhalKey, initial: Array<T> = emptyArray()) = NVhalProperty<Array<T>, Array<T>>(key, { it.value }).withInitial(initial)
        inline fun <Raw, reified T> array(key: NVhalKey, initial: Array<T> = emptyArray(), noinline transform: (Raw) -> T,) = NVhalProperty<Array<Raw>, Array<T>>(key, { it.value.map(transform).toTypedArray() }).withInitial(initial)
        ...
        fun <T> int(key: NVhalKey, initial: T, transform: (Int) -> T) = NVhalProperty(key, valueTransformer(transform)).withInitial(initial)
        ...
        fun boolean(key: NVhalKey, initial: Boolean = false) = NVhalProperty<Boolean, Boolean>(key, ::notrasnform).withInitial(initial)
        fun float(key: NVhalKey, initial: Float = 0f) = NVhalProperty<Float, Float>(key, ::notrasnform).withInitial(initial)
        fun float(key: NVhalKey, range: ClosedFloatingPointRange<Float>, initial: Float = 0f) = NVhalProperty<Float, RangedValue>(key) { RangedValue(it.value, range) }.withInitial(RangedValue(initial, range))
        ...
        fun <T> floatOutput(key: NVhalKey, transform: (T) -> Float) = NVhalFloatOutputProperty<T>(key, transform)
        ...
        fun <BaseUnit: MeasurementUnit> measurement(key: NVhalKey, unit: BaseUnit, initial: Float = 0f) = NVhalProperty(key, valueTransformer { raw: Float -> Measurement(raw, unit) }).withInitial(Measurement(initial, unit))
        fun <BaseUnit: MeasurementUnit> measurement(key: NVhalKey, unit: BaseUnit, range: MeasurementUnitRange<BaseUnit>, initial: Float = 0f) = NVhalProperty(key, valueTransformer { raw: Float -> MeasurementRanged(raw, unit, range) }).withInitial(MeasurementRanged(initial, unit, range))
        fun <Raw, T> raw(key: NVhalKey, transform: (CarPropertyValue<Raw>) -> T) = NVhalProperty(key, transform)
        fun <Raw, T> raw(key: NVhalKey, initial: T, transform: (CarPropertyValue<Raw>) -> T) = NVhalProperty(key, transform).withInitial(initial)
        fun <Raw, T> rawReducer(key: NVhalKey, initial: T, transform: (T, CarPropertyValue<Raw>) -> T): NVhalStateProperty<Raw, T> {
            var acc = initial
